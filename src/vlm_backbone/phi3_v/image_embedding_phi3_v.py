...
class Phi3ImageEmbedding(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, wte=None, **kwargs) -> None:
        ...
        # t2i_layers = [nn.Linear(image_dim_out, image_dim_out)]
        # t2i_layers.extend([nn.GELU(),
        #                     nn.Linear(image_dim_out, image_dim_out * 4)])

        # self.txt_projector = nn.Sequential(*t2i_layers)

        self.txt_projector = nn.Linear(image_dim_out, image_dim_out * 4, bias=True)
        # self.txt_projector.to_empty(device="cpu")
        ...
        if projection_cls == 'linear':
            self.img_projection = nn.Linear(image_dim_out, image_dim_out * 4)
    ...
    def get_img_features(self, img_embeds: torch.FloatTensor, t2i_embeds=None) -> torch.FloatTensor:
        LAYER_IDX = self.layer_idx
        TYPE_FEATURE = self.type_feature

        img_processor_output = self.img_processor(pixel_values=img_embeds, txt_values=t2i_embeds, output_hidden_states=True)
    ...
    def forward(
        ...
        # print(input_ids.shape, new_input_ids.shape)
        # print("pos", positions[0].tolist())
        ...
        if t2i_embeds is not None:
            # print('t2i shape', t2i_embeds.shape, hidden_states.shape)
            img_features = self.get_img_features(img_embeds=None, t2i_embeds=t2i_embeds)
            # print("img_features", img_features.shape)
            image_features_proj = self.img_projection(self.txt_projector(img_features))
            # print(image_features_proj.shape,hidden_states.shape)
            hidden_states = torch.cat((image_features_proj, hidden_states), dim=1)
            ...
            # print('image shape', image_features_proj.shape, hidden_states.shape)
            hidden_states = hidden_states.index_put(
                positions, image_features_proj, accumulate=False
            )
            # print(hidden_states.shape)
