...
from transformers import PreTrainedModel, AutoModelForCausalLM, AutoConfig, AutoModel, PhiModel, PhiConfig
...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        # print('Now attention.',attention_mask)
...
class Phi3FlashAttention2(Phi3Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("Now flash attention2.", attention_mask)
        ...
        # print(cos.shape, sin.shape, rotary_seq_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
...
class Phi3VModel(Phi3VPreTrainedModel):
    ...
    def __init__(self, config: Phi3VConfig):
        ...
        # print(config.t2i_config)
        self.t2i_model = PhiModel(PhiConfig.from_dict(config.t2i_config))
        # self.t2i_model.to_empty(device="cpu")
        self.t2i_proj = nn.Linear(2048, 1024, bias=True)
        # self.t2i_proj.to_empty(device="cpu")
        if isinstance(config.embd_layer, dict):
            ...
            # self.t2i_vit = Phi3ImageEmbedding(config, wte=self.embed_tokens, **embedding_config)
            # self.t2i_vit.to_empty(device="cpu")
    ...
    def forward(
        ...
        pix_ids: Optional[torch.LongTensor] = None,
        pix_mask: Optional[torch.Tensor] = None,
        ...
        # print("ids", input_ids, position_ids)
        ...
        if inputs_embeds is None:
            if pixel_values is not None and image_sizes is not None:
                ...
                # t2i_embeds = self.t2i_proj(self.t2i_model(inputs_embeds=inputs_embeds))
                batch_size, seq_length = pix_ids.shape[:2]
                position_ids = torch.arange(
                    past_key_values_length, seq_length + past_key_values_length, dtype=torch.long, device=device
                )
                position_ids = position_ids.unsqueeze(0).view(-1, seq_length)
                tmp = self.t2i_model(
                    input_ids=pix_ids,
                    attention_mask=pix_mask,
                    position_ids=position_ids,
                    past_key_values=past_key_values
                    ).last_hidden_state
                t2i_embeds = self.t2i_proj(tmp)
                # print('embeds', tmp, t2i_embeds)
                # inputs_embeds = self.embed_tokens(input_ids)
                # print(tmp)
                # vit
                # inputs_embeds = torch.cat((t2i_embeds, inputs_embeds), dim=1)
                inputs_embeds = self.vision_embed_tokens(input_ids, pixel_values=pixel_values, image_sizes=image_sizes, t2i_embeds=t2i_embeds)

                generated_img_tokens = inputs_embeds.shape[-2] - seq_length
                # update inputs
                # print(input_ids.shape, attention_mask.shape)
                input_ids = torch.cat((torch.full((batch_size, generated_img_tokens), fill_value=-1, device=input_ids.device), input_ids), dim=1)
                attention_mask = torch.cat((torch.full((batch_size, generated_img_tokens), fill_value=1, device=attention_mask.device), attention_mask), dim=1)
                # print(input_ids.shape, attention_mask.shape)
                batch_size, seq_length = input_ids.shape[:2]
                position_ids = torch.arange(
                    past_key_values_length, seq_length + past_key_values_length, dtype=torch.long, device=device
